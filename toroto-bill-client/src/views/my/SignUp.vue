<template>
  <div class="page">
    ...
    <div style="padding: 30% 20%;font-size: .42rem; background-color: white;">
      <p>加入金享汇，享受更多优质服务</p>
    </div>
...
export default {
  ...
  methods: {
    ...
    save() {
      ...
      this.$axios.post('userLogin/signUp', this.user).then((res) => {
        if (res.code == '000') {
          Toast.success('保存成功');
          this.$router.push({
            path: '/success',
            query: { phoneNumber: this.user.phoneNumber },
          });
        } else {
          Toast.fail(res.msg);
        }
